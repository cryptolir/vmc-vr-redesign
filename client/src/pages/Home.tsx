...
import { ChevronDown, Activity, ShieldAlert, Clock, Zap } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      {/* ── Section 1: Hero ──────────────────────────────────────────────── */}
      <section className="relative w-full flex items-center justify-center overflow-hidden" style={{ aspectRatio: "16 / 9", minHeight: "100vh" }}>
        <video autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center" }}>
        ...
        <div className="container">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
              {[
                { icon: Activity, title: "מתח גופני ורגשי", desc: "מטופלים מגיעים כשהגוף כבר במצב דריכות מוגברת — דופק מואץ, נשימה רדודה" },
                { icon: ShieldAlert, title: "קושי לשתף פעולה", desc: "קשה לצוות לעבוד עם מטופל שמערכת העצבים שלו במצב הגנתי" },
                { icon: Clock, title: "הארכת זמני טיפול", desc: "הדריכות מאריכה את הפעולה הרפואית וגוזלת זמן יקר מהצוות" },
                { icon: Zap, title: "תגובות לא צפויות", desc: "תגובות עוצמתיות במהלך הפעולה שניתן היה למנוע בהכנה מתאימה" },
              ].map((item, i) => {
                const IconComponent = item.icon;
                return (
                  <div key={i} className="p-8 bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-14 h-14 rounded-xl mb-5 flex items-center justify-center" style={{ background: "rgba(212, 149, 106, 0.10)", color: "var(--tension)" }}>
            ...
            <div className="flex flex-col items-center">
              <div className="w-16 border-t" style={{ borderColor: "var(--border)" }} />
              <p className="text-center text-lg font-semibold mt-16 max-w-2xl mx-auto leading-relaxed" style={{ color: "var(--calm)" }}>
                כל אחד מהקשיים האלה נפתר לפני שהמטופל נכנס לחדר הטיפול.
              </p>
            </div>
          </div>
        </div>
